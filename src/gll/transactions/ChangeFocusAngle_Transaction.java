 ...
 * @author mirza
 */
public class ChangeFocusAngle_Transaction implements jTPS_Transaction {
    GoLogoLoApp app;
    Shape EditedNode;
    Paint fill;
    Paint Changedfill;
    ...
    public ChangeFocusAngle_Transaction(GoLogoLoApp initapp,Shape initnode) {
        app = initapp;
        EditedNode = initnode;
    ...
    public void doTransaction() {
        if(Changedfill==null){
            double slidervalue = ((Slider)app.getGUIModule().getGUINode(GLL_FOCUS_SLIDER)).getValue()*3;
            RadialGradient grad = new RadialGradient(slidervalue,distance,centerX,centerY,radius,true,method,stops);
