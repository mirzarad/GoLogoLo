 ...
 * @author mirza
 */
public class ChangeCenterX_Transaction implements jTPS_Transaction {
    GoLogoLoApp app;
    Shape EditedNode;
    Paint fill;
    Paint Changedfill;
    ...
    public ChangeCenterX_Transaction(GoLogoLoApp initapp,Shape initnode) {
        app = initapp;
        EditedNode = initnode;
    ...
    public void doTransaction() {
        if(Changedfill==null){
            double slidervalue = ((Slider)app.getGUIModule().getGUINode(GLL_CENTER_X_SLIDER)).getValue()*0.2;
            RadialGradient grad = new RadialGradient(focus,distance,slidervalue,centerY,radius,true,method,stops);
