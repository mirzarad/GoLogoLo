 ...
 * @author mirza
 */
public class ChangeStop1Color_Transaction  implements jTPS_Transaction {
    GoLogoLoApp app;
    Shape EditedNode;
    Paint fill;
    Paint Changedfill;
    ...
    public ChangeStop1Color_Transaction(GoLogoLoApp initapp,Shape initnode) {
        app = initapp;
        EditedNode = initnode;
        ...
        if(EditedNode.getFill() instanceof RadialGradient){
            ...
            stops.add(((RadialGradient)EditedNode.getFill()).getStops().get(0));
        }
        else{
            angle = 0;
            distance = 0;
    ...
    public void doTransaction() {
        if(Changedfill==null){
            Color color = ((ColorPicker)app.getGUIModule().getGUINode(GLL_STOP1_COLOR_PICKER)).getValue();
